...
-   <xref:System.Activities.Statements.Transition>  
 ...
 상태 시스템 워크플로를 만들기 위해 상태가 <xref:System.Activities.Statements.StateMachine> 활동에 추가되고 상태 간 흐름을 제어하기 위해 전환이 사용됩니다. 다음 스크린샷에서에서 합니다 [초보자를 위한 자습서](../../../docs/framework/windows-workflow-foundation/getting-started-tutorial.md) 단계 [방법: 상태 시스템 워크플로 만드는](../../../docs/framework/windows-workflow-foundation/how-to-create-a-state-machine-workflow.md), 세 가지 상태 및 세 가지 전환을 사용 하 여 상태 시스템 워크플로 보여 줍니다. **Initialize Target** 워크플로에서 첫 번째 상태를 나타내며 초기 상태입니다. 해당 상태로 이끄는 선으로 지정 하는이 **시작** 노드. 워크플로의 최종 상태 라고 **FinalState**, 워크플로가 완료 되는 시점을 나타냅니다.  
