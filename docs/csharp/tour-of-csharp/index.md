...
일부 C# 기능은 강력하고 안정적인 애플리케이션의 생성에 도움을 줍니다. 즉, ***가비지 수집***은 연결할 수 없는 미사용 개체가 차지하는 메모리를 자동으로 회수합니다. ***예외 처리***는 오류 감지 및 복구에 대한 체계적이고 확장 가능한 접근 방식을 제공합니다. 언어의 ***형식 안전*** 설계는 초기화되지 않은 변수에서 읽을 수 없게 하고, 범위를 초과하는 배열을 인덱싱할 수 없게 하고, 확인되지 않은 형식 캐스트를 수행할 수 없게 합니다.
