---
title: 새로운&#39;Windows Communication Foundation 4.5 기능
ms.date: 03/30/2017
helpviewer_keywords:
- WCF [WCF], what's new
...
ms.locfileid: "44085993"
---
# <a name="what39s-new-in-windows-communication-foundation-45"></a>새로운&#39;Windows Communication Foundation 4.5 기능
이 항목에서는 Windows Communication Foundation (WCF)에 새로운 기능을 설명합니다.  
...
## <a name="websocket-support"></a>WebSocket 지원  
 WebSocket은 TCP와 유사한 성능 특성으로 포트 80 및 443에서 진정한 의미의 양방향 통신을 제공하는 기술입니다. WebSocket 전송에서 통신을 지원하기 위해 <xref:System.ServiceModel.NetHttpBinding>와 <xref:System.ServiceModel.NetHttpsBinding>을 참조하세요. 자세한 내용은 참조: [System-Provided Bindings](../../../docs/framework/wcf/system-provided-bindings.md)합니다.  
...
|receiveTimeout|SMSvcHost.exe|30초|[Net.TCP Port Sharing Service 구성](https://msdn.microsoft.com/library/b6dd81fa-68b7-4e1b-868e-88e5901b7ea0)|  
...
## <a name="webservicehost-and-default-endpoints"></a>WebServiceHost 및 기본 엔드포인트  
 Visual Studio 2010에서는 사용자가 엔드포인트를 명시적으로 지정했는지 여부에 상관 없이 WebServiceHost가 기본 엔드포인트를 자동으로 만들었습니다. Visual Studio 2012에서는 엔드포인트가 명시적으로 추가되지 않은 경우에만 WebServiceHost가 기본 엔드포인트를 만듭니다. 클라이언트에 기본 엔드포인트가 필요한 경우에는 엔드포인트를 명시적으로 추가하고 클라이언트를 이 엔드포인트에 연결할 수 있습니다. 또는 응용 프로그램의 구성 파일에 다음 설정을 추가하여 WCF가 이전 동작으로 되돌아가도록 지정할 수도 있습니다.  
