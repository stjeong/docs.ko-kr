...
## <a name="example"></a>예제  
 ...
 첫 번째 `MsgBox` 표시 호출 "increase(n) 후: 11, 21, 31, 41"입니다. 때문에 배열의 `n` 참조 형식인 `increase` 전달 메커니즘이 해당 멤버를 변경할 수 있습니다 `ByVal`합니다.  
  
 두 번째 `MsgBox` 표시 호출 "replace(n) 후: 11, 21, 31, 41"입니다. 때문에 `n` 되는지를 `ByVal`를 `replace` 변수를 수정할 수 없습니다 `n` 에 새 배열을 할당 하 여 호출 코드에서. 때 `replace` 배열의 새 인스턴스를 만듭니다 `k` 로컬 변수에 할당 하 고 `a`에 대 한 참조를 잃어 `n` 호출 코드에서 전달 합니다. 멤버를 변경할 때 `a`, 로컬 배열만 `k` 영향을 받습니다. 따라서 `replace` 배열의 값이 증가 하지 `n` 호출 코드에서.  
