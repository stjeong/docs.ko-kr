...
.NET Framework에서 코드를 실행한 적이 있으면 .NET Core에서 코드를 실행하는 데 관심이 있을 수 있습니다.  이 문서에서는 이식 프로세스 및 .NET Core로 이식할 때 유용하게 사용할 수 있는 도구 목록에 대해 간략하게 설명합니다.
...
2. .NET Framework 최신 버전으로 이식하려는 모든 프로젝트의 대상을 다시 지정합니다.
...
4. 테스트 코드를 이식합니다.
   ...
   .NET Core로 이식하면 코드베이스가 너무 많이 변경되기 때문에 코드를 이식할 때 테스트를 실행할 수 있도록 테스트를 이식하는 것이 가장 좋습니다.  현재 MSTest, xUnit 및 NUnit 모두 .NET Core를 지원합니다.
