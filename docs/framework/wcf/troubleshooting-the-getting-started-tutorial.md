...
**클라이언트 응용 프로그램을 컴파일: 'CalculatorClient'에 대 한 정의가 없습니다. '\<메서드 이름 >' 및 확장 메서드 '\<메서드 이름 >' 수락 'CalculatorClient' 형식의 첫 번째 인수를 찾을 수 없습니다 (이십니까 using 지시문 또는 어셈블리 참조가?)**  
...
**클라이언트를 실행 합니다: 처리 되지 않은 예외: System.ServiceModel.EndpointNotFoundException:에 연결 하지 못했습니다 `http://localhost:8000/ServiceModelSamples/Service/CalculatorService`합니다. TCP 오류 코드 10061: 대상 컴퓨터에서 적극적으로 거부 했기 때문에 없는 연결을 설정할 수 없습니다.**
