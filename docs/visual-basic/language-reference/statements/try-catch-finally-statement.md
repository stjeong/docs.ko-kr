...
-   [End 문](../../../visual-basic/language-reference/statements/end-statement.md) 발견 되는 `Try` 또는 `Catch` 블록입니다.  
...
> [!TIP]
>  특정 예외를 catch 하지 않은 경우는 `Using` 문 처럼는 `Try…Finally` 블록과 블록을 종료 하는 방법에 관계 없이 리소스를 삭제할 수 있습니다. 처리 되지 않은 예외를 사용 하 여도 마찬가지입니다. 자세한 내용은 [using 문](../../../visual-basic/language-reference/statements/using-statement.md)을 참조하세요.  
...
## <a name="async-methods"></a>비동기 메서드  
 메서드를 사용 하 여 표시 하는 경우는 [비동기](../../../visual-basic/language-reference/modifiers/async.md) 한정자를 사용할 수는 [Await](../../../visual-basic/language-reference/operators/await-operator.md) 메서드에서 연산자입니다. 사용 하 여 문을 여 `Await` 연산자는 대기 중인된 작업이 완료 될 때까지 메서드의 실행을 일시 중단 합니다. 작업은 진행 중인 작업을 나타냅니다. 경우 연관 된 태스크는 `Await` 연산자 완료 되 면 동일한 메서드에서 실행 다시 시작 합니다. 자세한 내용은 [비동기 프로그램의 제어 흐름](../../../visual-basic/programming-guide/concepts/async/control-flow-in-async-programs.md)합니다.  
...
## <a name="iterators"></a>반복기  
 반복기 함수 또는 `Get` 접근자 컬렉션 사용자 지정 반복을 수행 합니다. 반복기를 사용 하는 [Yield](../../../visual-basic/language-reference/statements/yield-statement.md) 문을 한 번에 하나씩 컬렉션의 각 요소를 반환 합니다. 반복기 함수를 사용 하 여 호출을 [각각에 대 한 중... 다음 문을](../../../visual-basic/language-reference/statements/for-each-next-statement.md)합니다.  
