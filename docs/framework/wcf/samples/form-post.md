...
5.  그런 다음 고객 폼의 URI로 이동할지 묻는 메시지가 표시됩니다. 브라우저를 열고 지정된 URI로 이동합니다. 이름 및 고객과 클릭에 대 한 주소를 입력 합니다 **제출** 단추입니다.  
