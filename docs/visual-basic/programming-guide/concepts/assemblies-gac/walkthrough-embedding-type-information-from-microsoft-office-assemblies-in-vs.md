...
6.  **CreateExcelWorkbook** 프로젝트에 대한 참조 목록에서, 이전 단계에서 추가한 `Microsoft.Office.Interop.Excel`에 대한 참조를 선택합니다. **속성** 창에서 `Embed Interop Types` 속성이 `True`로 설정되어 있는지 확인합니다.  
    ...
    > [!NOTE]
    >  이 연습에서 만든 애플리케이션은 포함된 interop 형식 정보 때문에 서로 다른 버전의 Microsoft Office에서 실행됩니다. `Embed Interop Types` 속성을 `False`로 설정한 경우 응용 프로그램을 실행할 각 Microsoft Office 버전용 PIA를 포함해야 합니다.  
...
9. Ctrl+F5를 눌러 프로젝트를 빌드하고 실행합니다. Excel 통합 문서 예제 코드에서 지정한 위치에 만들어졌는지 확인 합니다. C:\SampleFolder\SampleWorkbook.xls.  
...
5.  Excel 통합 문서에 샘플 코드에 지정 된 위치에 만들어졌는지 확인 합니다. C:\SampleFolder\SampleWorkbook.xls.  
