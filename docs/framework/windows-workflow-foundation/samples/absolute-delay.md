...
>  <xref:System.DateTime>에서 <xref:System.TimeSpan>으로 변환하면 정확도가 약간 손실됩니다.  
 ...
 이 샘플을 단계별로 기본 제공 지연이 시작 및 완료 된에 전자 메일 메시지를 전송 하면 표시 됩니다. 여기서 AbsoluteDelay 작업은 지정한 <xref:System.DateTime>(또는 <xref:System.DateTime>이 만료된 경우 0초) 전까지 중지되며, 만료 시 전자 메일이 전송됩니다. 이 샘플에서는 기본 제공 <xref:System.Activities.Statements.Delay> 기능의 두 가지 사용 사례를 AbsoluteDelay 작업 사용과 비교하여 보여 줍니다.  
...
7.  선택 하 여 솔루션을 빌드합니다 **빌드**하십시오 **솔루션 빌드**합니다.  
