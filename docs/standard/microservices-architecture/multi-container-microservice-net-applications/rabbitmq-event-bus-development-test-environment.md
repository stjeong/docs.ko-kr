...
다음 코드는 RabbitMQ에 대한 간소화된 이벤트 버스 구현의 일부이며 eShopOnContainers의 [실제 코드](https://github.com/dotnet-architecture/eShopOnContainers/blob/master/src/BuildingBlocks/EventBus/EventBusRabbitMQ/EventBusRabbitMQ.cs)를 향상시킵니다. 일반적으로 개선하는 경우가 아니면 코딩할 필요가 없습니다. 코드는 RabbitMQ에 대한 연결 및 채널을 보유하고, 메시지를 만들고, 큐에 메시지를 게시합니다.
