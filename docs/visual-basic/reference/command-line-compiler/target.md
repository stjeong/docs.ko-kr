...
|`-target:appcontainerexe`|컴파일러가 실행 Windows 기반 응용 프로그램을 만들려면 앱 컨테이너에서 실행 해야 하는 합니다. 이 설정은 사용자에 사용할 [!INCLUDE[win8_appname_long](~/includes/win8-appname-long-md.md)] 응용 프로그램입니다.<br /><br /> 합니다 **appcontainerexe** 설정 비트의 특성 필드를 설정 합니다 [이식 가능한 실행](https://msdn.microsoft.com/library/windows/desktop/ms680547(v=vs.85).aspx?id=19509) 파일입니다. 이 비트는 앱 컨테이너에서 앱을 실행 해야 함을 나타냅니다. 이 비트가 설정 된 경우 오류가 발생 하는 경우는 `CreateProcess` 메서드는 앱 컨테이너 외부에서 응용 프로그램을 시작 하려고 합니다. 이 비트를 설정 하는 것 외 **-target: appcontainerexe** 같습니다 **-/target: winexe**합니다.<br /><br /> 실행 파일은 확장명이.exe 인 생성 됩니다.<br /><br /> 사용 하 여 별도로 지정 하지 않으면 합니다 `-out` 옵션을 출력 파일 이름을 포함 하는 입력된 파일의 이름을 사용 합니다 `Sub Main` 프로시저입니다.<br /><br /> 하나의 `Sub Main` .exe 파일로 컴파일되는 소스 코드 파일의 절차가 필요 합니다. 코드에 있는 둘 이상의 클래스를 포함 하는 경우는 `Sub Main` 프로시저를 사용 하 여 합니다 `-main` 컴파일러 옵션을 포함 하는 클래스를 지정 합니다 `Sub Main` 프로시저|  
