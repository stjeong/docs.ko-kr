...
3.  새 인스턴스를 만들어 가장을 시작 합니다 <xref:System.Security.Principal.WindowsImpersonationContext> 클래스를 만들고 사용 하 여 초기화를 <xref:System.Security.Principal.WindowsIdentity.Impersonate%2A?displayProperty=nameWithType> 다음 코드에 표시 된 것 처럼 초기화 클래스의 메서드.  
...
4.  더 이상 가장할 필요가 호출을 <xref:System.Security.Principal.WindowsImpersonationContext.Undo%2A?displayProperty=nameWithType> 메서드를 다음 코드에 표시 된 대로 가장을 되돌립니다.  
