...
|proxyAuthenticationScheme|HTTP 프록시가 처리하는 클라이언트 요청을 인증하는 데 사용되는 프로토콜을 지정합니다. 유효한 값은 다음과 같습니다.<br /><br /> -None. 인증이 수행되지 않습니다.<br />-요약: 다이제스트 인증을 지정합니다.<br />-Negotiate: 클라이언트와 협상하여 인증 체계를 결정합니다. 클라이언트와 서버 모두 Kerberos를 지원하면 이 인증 체계가 사용되고, 그렇지 않으면 NTLM이 사용됩니다.<br />-Ntlm. NTLM 인증을 지정합니다.<br />-기본: 기본 인증을 지정합니다.<br />익명: 익명 인증을 지정합니다.<br />-IntegratedWindowsAuthentication: Windows 인증을 지정합니다.<br /><br /> 기본값은 Anonymous입니다. 이 특성은 <xref:System.Net.AuthenticationSchemes> 형식입니다.|  
