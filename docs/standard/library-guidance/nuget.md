...
> SourceLink는 `RepositoryUrl` 및 `RepositoryType` 메타데이터를 NuGet 패키지에 자동으로 추가합니다. SourceLink는 패키지가 빌드된 정확한 소스 코드에 대한 정보도 추가합니다. 예를 들어 Git 리포지토리에서 만든 패키지에는 메타데이터로 추가된 커밋 해시가 있습니다.
