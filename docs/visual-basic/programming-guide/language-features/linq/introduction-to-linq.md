...
|LINQ to DataSet|LINQ to DataSet 공급자에서 데이터를 쿼리 및 업데이트 하면는 [!INCLUDE[vstecado](~/includes/vstecado-md.md)] 데이터 집합입니다. 데이터 집합의 데이터를 쿼리, 집계 및 업데이트하는 기능을 단순화하고 확장하기 위해 데이터 집합을 사용하는 응용 프로그램에 LINQ의 기능을 추가할 수 있습니다.<br /><br /> 자세한 내용은 [LINQ to DataSet](../../../../framework/data/adonet/linq-to-dataset.md)를 참조하세요.|  
