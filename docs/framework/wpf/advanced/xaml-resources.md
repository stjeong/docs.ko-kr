...
> [!NOTE]
>  이 항목에서 설명 하는 리소스 파일은 리소스 파일에 설명 된 다르지 [WPF 응용 프로그램 리소스, 콘텐츠 및 데이터 파일](../../../../docs/framework/wpf/app-development/wpf-application-resource-content-and-data-files.md) 포함 또는 연결 된 리소스에 설명 된 것과 다르며 [관리 응용 프로그램 리소스 (.NET)](/visualstudio/ide/managing-application-resources-dotnet)합니다.  
