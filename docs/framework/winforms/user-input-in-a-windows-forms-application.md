...
|<xref:System.Windows.Forms.Control.OnNotifyMessage%2A>|이 메서드는 처리 된 후 폼과 컨트롤 수준에서 메시지를 차단 합니다. <xref:System.Windows.Forms.ControlStyles.EnableNotifyMessage> 이 메서드를 호출할 수에 대 한 스타일 비트를 설정 해야 합니다.|  
 ...
 키보드 및 마우스 메시지가 해당 유형의 메시지에만 적용 되는 재정의 가능한 메서드 집합을 추가 하 여 처리 됩니다. 자세한 내용은 [키보드 입력 작동 방식](../../../docs/framework/winforms/how-keyboard-input-works.md) 하 고 [Windows Forms의 마우스 입력 방법](../../../docs/framework/winforms/how-mouse-input-works-in-windows-forms.md)합니다.  
